...
# 1. Configuración de página (Esto causaba el NameError si no había 'import streamlit as st')
st.set_page_config(page_title="Facturador", layout="wide")

# Ocultar decoración (Quita el cohete)
...
# --- CABECERA (Nº Factura de 5000 cifras y Fecha) ---
...
# --- GENERADOR DE PDF (Sin símbolos que den error Unicode) ---
def crear_pdf():
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.cell(0, 8, f"Número: {num_factura}  |  Fecha: {fecha_factura}", 0, 1, 'R')
    pdf.ln(10)
    pdf.cell(0, 10, f"TOTAL NETO: {total:.2f} EUR", 0, 1)
    pdf.cell(0, 10, f"IBAN: {mi_nombre} - {mi_iban}", 0, 1)
